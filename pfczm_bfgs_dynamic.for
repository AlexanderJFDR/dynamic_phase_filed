...
!**********************************************************************************************************
        ...
        ! 
        real(rkind) :: thk, EA, nu, Gf, ft, lb, rho
        real(rkind) :: De(3, 3)        
...
!**********************************************************************************************************
!
      subroutine pfczm(rhs,amatrix,coords,u,v,a,svars,params,energy,dtime,lflags)
!
!**********************************************************************************************************
        use NumKind
        use ModelParam
        ...
        real(rkind):: svars(12), u(12), v(12), a(12)
		real(rkind):: params(3), energy(8), dtime
		integer(ikind):: lflags(*)
       
        ! local varibles
        real(rkind):: nn(2,8), b(3,8), nd(4), bd(2,4)
        real(rkind):: uu(8), vv(8), aa(8), dd(4), vd(4), rd(4), ru(8), kdd(4,4), kuu(8,8), rd_temp(4)
		real(rkind):: Am(8,8), Ak(8,8), Ac(8,8)
        real(rkind):: rr(12), kk(12,12)
        real(rkind):: strain(3), stressEff(3)
        real(rkind):: det_jacb, energy_crk, phi, v_phi, omega, domega, ddomega
        real(rkind):: aalpha, dalpha, ddalpha, phi_source, dphi_source, dvol
		real(rkind):: alpha, beta, gamma, dadu, dvdu, dynt6, dynt4
        integer(ikind):: i, j, k
...
c           max/min pricipal stress
			...
			v_phi = dot_product(nd,dd)
            call geometricFunc(aalpha,dalpha,ddalpha,phi) ! geometric function
     ...
     &          +  2.d0*lb*Gf/c0*matmul(transpose(bd),bd))
            ...
            Am =  Am + dvol*rho*matmul(transpose(nn), nn)
			Ac =  Ac + dvol*rho*domega*v_phi*matmul(transpose(nn), nn)
			Ak =  Ak + dvol*matmul(matmul(transpose(b), omega*De), b)
			
			if (lflags(1).EQ.1 .OR. lflags(1).EQ.2) then
			  energy(1) = 0.d0
		      energy(2) = energy(2) + 0.5d0*dvol*omega*dot_product(stressEff,strain)
		      energy(5) = energy(5) + dvol*Gf/c0*(aalpha/lb+lb*dot_product(matmul(bd, dd), matmul(bd, dd)))
			else if (lflags(1).EQ.11 .OR. lflags(1).EQ.12) then
			  energy(1) = energy(1) + 0.5d0*dvol*rho*dot_product(matmul(nn, vv), matmul(nn, vv))
		      energy(2) = energy(2) + 0.5d0*dvol*omega*dot_product(stressEff,strain)
		      energy(5) = energy(5) + dvol*Gf/c0*(aalpha/lb+lb*dot_product(matmul(bd, dd), matmul(bd, dd)))	
			end if
          end do
...
C                  
          dadu = 1.d0/(beta*dtime**2.d0)
		  dvdu = gamma/(beta*dtime)
		  
		!  Ac = 0.0006d0*Am + 0.00026d0*Ak
          
          kuu = DADU*Am + dvdu*(1.d0+alpha)*Ac +(1.d0+alpha)*Ak
		  ru = -matmul(Am,aa) -(1.d0+alpha)*matmul(Ac,vv) - (1.d0+alpha)*matmul(Ak,uu) + alpha*svars(5:12)	
	 
        end if
		
		svars(5:12) = matmul(Ac,vv)+matmul(Ak,uu)
...
!**********************************************************************************************************
!
      subroutine geometricFunc(aalpha,dalpha,ddalpha,phi)
!
!**********************************************************************************************************
        use NumKind
        implicit none
        
        real(rkind) :: aalpha, dalpha, phi, ddalpha
        
		aalpha  = 2.d0*phi - phi**2.d0
...
!**********************************************************************************************************
     ...
     &    amatrx(ndofel,ndofel), svars(nsvars), energy(8)
      ...
      !
        ...
        call pfczm(rhs(:,1),amatrx,coords,u,v,a,svars,params,energy,dtime,lflags)
